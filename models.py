    ...
    def forward(self, triples, mode='train'):
        ...
        if mode == 'train':
            ...
            if self.remove_one_loop:
                filter_index = self.edge_match(self.edge_data.T[[0, 2], :], extend_triples[[0, 2], :])[0]  # for FB
            else:
                filter_index = self.edge_match(self.edge_data.T, extend_triples)[0]
            filter_mask = ~self.index_to_mask(filter_index, len(self.edge_data))
